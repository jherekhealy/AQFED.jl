...
using Roots
import AQFED.Math: quadRootsReal, normcdf, normpdf, norminv, inv, ClosedTransformation, FitResult
...
struct ExpBSplineCollocation{T,U}
    g::PP{2,T,U} #a bspline, can I express price directly in terms of bspline basis?
    forward::U
end
...
function solvePositiveStrike(c::ExpBSplineCollocation{U,T}, expStrike::T)::Tuple{U,Int} where {U, T <: Number}
    ...
    return solveStrikeAt(pp,i,strike)
end
...
function firstMomentExtrapolationBounded(p::ExpBSplineCollocation{T,U}, i::Int, x0t::Union{U,T}, x1t::Union{U,T})::T where {T,U}
    pp = p.g
...
function firstMomentBounded(p::ExpBSplineCollocation{T,U}, i::Int, a::Union{U,T}, b::Union{U,T})::T where {T,U}
    pp = p.g
    x0 = pp.x[i]
    x1 = pp.x[i+1]
    x0t = max(a, x0)
...
function hermiteIntegralBounded(p::ExpBSplineCollocation{T,U}, ck::Union{T,U}, ckIndex::Int)::T where {T,U}
    pp = p.g
    n = length(pp.x)
    i = ckIndex
    if i > length(pp.x)
...
function fitExpBSplineBachelier(strikes, prices, weights, τ, forward, discountDf; size::Int = 0, slopeTolerance::Float64 = sqrt(eps(Float64)))
    ...
    if size == 0
        ...
        # x = collect(range(start=xf[1],stop=max(xf[end],1.5),length=n))
    end
    # println("x=", x, " ",length(x))
...
function fit(isoc::ExpBSplineCollocation{TZ,TX}, strikes, prices, weights, forward, discountDf; minSlope = 1e-8, penalty = 0.0) where {TZ,TX}
    iter = 0
