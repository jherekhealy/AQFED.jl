...
function hermiteIntegralBounded(p::ExpBSplineCollocation{T,U}, ck::Union{U,T}, ckIndex::Int)::T where {T,U}
    pp = p.g
    n = length(pp.x)
    i = ckIndex
    if i > length(pp.x)
...
function fit(isoc::ExpBSplineCollocation{TZ,TX}, strikes, prices, weights, forward, discountDf; minSlope = 1e-8, penalty = 0.0) where {TZ,TX}
    ...
    function obj!(fvec, ct0::AbstractArray{T}) where {T}
        ...
        pp = convert(QuadraticPP{T,TX}, spl)
...
#     end
    ...
    pp = convert(QuadraticPP{TZ,TX}, spl)
