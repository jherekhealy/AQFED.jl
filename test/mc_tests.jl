...
@testset "DoubleHestonDVSS2X" begin
    gens = [
        AQFED.Random.MersenneTwister64(UInt64(20130129)),
        AQFED.Random.Mixmax17(UInt64(20130129)),
        AQFED.Random.Well1024a(UInt32(20130129)),
    ...
    for (i, tte) = enumerate(τs)
        ...
        for (j, strike) = enumerate(strikes)
            ...
            value, serr =
                AQFED.MonteCarlo.simulateDVSS2X(AbstractRNGSeq(rng, nd), hParams, spot, payoff, 0, 1024 * 64, 0.16)
            println(tte, " ", strike, " ", value, " ", serr)
            @test isapprox(refPrices[i, j], value, atol=serr * 2)
        end
    end
end
using QuadGK, Roots, PPInterpolation, ArbNumerics, CharFuncPricing

@testset "DoubleHestonVVIX" begin
    kappa1 = 0.1906
    theta1 = 0.01909
    sigma1 = 0.03791
    v01 = 0.03584
    ...
    #calibration approximate from 2024 vs
    kappa1 = 2.935
    theta1 = 0.0131
    sigma1 = 2.429
    v01 = 0.0221
    ...
    v02 = 0.0221
    rho2 = -0.891
    #calibration approximate from 2024 fixing kappa + vs (better fit since kappa is not from vs fully)
    kappa1 = 0.75
    theta1 = 0.0481
    sigma1 = 0.411
    v01 = 0.0235
    ...
    params1 = HestonParams(ArbFloat(v01), ArbFloat(kappa1), ArbFloat(theta1), ArbFloat(rho1), ArbFloat(sigma1))
    params2 = HestonParams(ArbFloat(v02), ArbFloat(kappa2), ArbFloat(theta2), ArbFloat(rho2), ArbFloat(sigma2))
    kappa1 = 2.5849
    theta1 = 0.02460
    sigma1 = 1.1445
    v01 = 0.01464
    ...
    params = DoubleHestonParams(params1, params2)

    paramsd = DoubleHestonParams{Float64}(HestonParams{Float64}(0.02360551743821593, 0.004425820849239148, 1.372063677047353, -0.9899999996147979, 0.12261299034550824), HestonParams{Float64}(0.02360551743821593, 0.6291851978327323, 0.00010002356323646211, -0.9899999999995235, 2.3406201494627266))

    params1 = HestonParams(ArbFloat(paramsd.heston1.v0), ArbFloat(paramsd.heston1.κ), ArbFloat(paramsd.heston1.θ), ArbFloat(paramsd.heston1.ρ), ArbFloat(paramsd.heston1.σ))
    params2 = HestonParams(ArbFloat(paramsd.heston2.v0), ArbFloat(paramsd.heston2.κ), ArbFloat(paramsd.heston2.θ), ArbFloat(paramsd.heston2.ρ), ArbFloat(paramsd.heston2.σ))
    params = DoubleHestonParams(params1, params2)
    upperBound1 = 1.0 #10 * params1.σ^2
    upperBound2 = 10 * params2.σ^2
    vixFuture = quadgk(x -> quadgk(
            y -> sqrt(
                AQFED.VolatilityModels.integratedVarianceExpectation(params1, x) +
                AQFED.VolatilityModels.integratedVarianceExpectation(params2, y)) *
                 AQFED.VolatilityModels.hestonDensity(params2, y, dt=30 / 365), 0, upperBound2, rtol=1e-6)[1] *
                            AQFED.VolatilityModels.hestonDensity(params1, x, dt=30 / 365), 0, upperBound1, rtol=1e-4)[1]

    valueRef = quadgk(x -> quadgk(y -> -log(sqrt(AQFED.VolatilityModels.integratedVarianceExpectation(params1, x) +
                                                 AQFED.VolatilityModels.integratedVarianceExpectation(params2, y))) *
                                       AQFED.VolatilityModels.hestonDensity(params2, y, dt=30 / 365), 0, upperBound2, rtol=ArbFloat(1e-6))[1] *
                           2 * 365 / 30 * AQFED.VolatilityModels.hestonDensity(params1, x, dt=30 / 365), 0.0, upperBound1, rtol=ArbFloat(1e-4))[1] + 365 / 30 * 2 * log(vixFuture)
    vvix = sqrt(valueRef) * 100

    vixFuture = quadgk(x -> quadgk(
            y -> sqrt(
                AQFED.VolatilityModels.integratedVarianceExpectation(params1, exp(x)) +
                AQFED.VolatilityModels.integratedVarianceExpectation(params2, exp(y))) *
                 AQFED.VolatilityModels.hestonLogDensity(params2, y, dt=30 / 365), ArbFloat(-1000000.0), ArbFloat(100.0), rtol=1e-20)[1] *
                            AQFED.VolatilityModels.hestonLogDensity(params1, x, dt=30 / 365), ArbFloat(-1000000.0), ArbFloat(100.0), rtol=1e-16)[1]
    valueRef = quadgk(x -> quadgk(y -> -log(sqrt(AQFED.VolatilityModels.integratedVarianceExpectation(params1, exp(x)) +
                                                 AQFED.VolatilityModels.integratedVarianceExpectation(params2, exp(y)))) *
                                       AQFED.VolatilityModels.hestonLogDensity(params2, y, dt=30 / 365), ArbFloat(-1000000.0), ArbFloat(100.0), rtol=ArbFloat(1e-20))[1] *
                           2 * 365 / 30 * AQFED.VolatilityModels.hestonLogDensity(params1, x, dt=30 / 365), ArbFloat(-1000000.0), ArbFloat(100.0), rtol=ArbFloat(1e-16))[1] + 365 / 30 * 2 * log(vixFuture)
    vvix = sqrt(valueRef) * 100
    #Note: HCubature is not precise enough or too slow, using log transform is important, even if the interval is huge. A good test is to check that the density integrates to 1.0

end

@testset "HestonVVIXTheo" begin
    #calibration Heston08October2024 inv vega weighted. to variance swaps
    paramMap = Dict()
    paramMap["Heston VS"] = HestonParams(0.04317461510443543, 0.00010014522603252196, 18.246672269950334, -0.9279241182416658, 0.1992284531767838)
    paramMap["Heston inverse vega weights"] = HestonParams{Float64}(0.037550363823728755, 3.439078585104335, 0.05081873488176465, -0.760708191981651, 1.8395586268919135)
    paramMap["Heston uniform weights"] = HestonParams{Float64}(0.024166492665314742, 0.2859069209434172, 0.08283476755892016, -0.7464447504624696, 0.34598000440430204)
    for (name, paramsf) = paramMap
        params = HestonParams(ArbFloat(paramsf.v0), ArbFloat(paramsf.κ), ArbFloat(paramsf.θ), ArbFloat(paramsf.ρ), ArbFloat(paramsf.σ))
        g = function (x)
        ...
        vixfut = quadgk(x -> vixCall(0.0, 1.0, x), ArbFloat(0.0), 100.0 * params.σ^2, rtol=ArbFloat(1e-8))[1]
        valueRef = quadgk(g, ArbFloat(0.0), 100.0 * params.σ^2, rtol=ArbFloat(1e-8))[1] + 365 / 30 * 2 * log(vixfut)
        println(name, " VVIX Full-Range ", sqrt(valueRef[1]) * 100, "VIX Future ", vixfut * 100)
    end
...
#=
vsTheo = [AQFED.VolatilityModels.priceVarianceSwap(AQFED.VolatilityModels.FukasawaVarianceSwapReplication(true), t, log.(strikes / uForwards[i]), vols[i, :] .^ 2, 1.0) for (i, t) = enumerate(ts)]
vsTheoFlat = [AQFED.VolatilityModels.priceVarianceSwap(AQFED.VolatilityModels.FukasawaVarianceSwapReplication(false), t, log.(strikes / uForwards[i]), vols[i, :] .^ 2, 1.0) for (i, t) = enumerate(ts)]

tFine = range(ts[1]/2,ts[end],length=1000)
p1 = plot(ts,vsTheo, label="Replication linear extrap.", seriestype=:scatter)
plot!(ts,vsTheoFlat, label="Replication flat extrap.", seriestype=:scatter)
for (name, paramsf) = paramMap
plot!(p1, tFine, [AQFED.VolatilityModels.integratedVarianceExpectation(paramsf, paramsf.v0; dt=t)*100^2 for t = tFine], label=name)
end
=#
end
